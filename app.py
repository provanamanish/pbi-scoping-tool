...
def ensure_dependencies():
    """Install missing dependencies required for Flask and DAX extraction"""
    ...
    for package in required_packages:
        try:
            ...
            try:
                ...
                print(f"[*] {package} not available (will use fallback for DAX extraction)")
    
    # Check for pbixray (already installed by user)
    try:
        __import__('pbixray')
        print(f"[OK] pbixray is available")
    except ImportError:
        print(f"[INFO] pbixray not found (will use DataModelSchema fallback for DAX extraction)")
...
def scope():
    ...
    if session.get("same_file", False):
        ...
        # Still extract DAX expressions even for same file
        print(f"[DEBUG] Extracting DAX for: {session['old_path']}")
        dax_map = pbi.extract_dax_expressions(session["old_path"])
        print(f"[DEBUG] DAX map has {len(dax_map)} entries")
    ...
    # DEBUG: Log DAX extraction status
    if rows:
        dax_count = sum(1 for r in rows if r.get("dax_expression", "").strip())
        print(f"[DEBUG] Returned {len(rows)} rows, {dax_count} have DAX expressions")
...
if __name__ == "__main__":
    ...
    # Note: debug=False to avoid subprocess encoding issues on Windows
    # If you need debug mode, edit this line or set FLASK_DEBUG=1
    # Note: host='127.0.0.1' for proper Windows localhost binding
    # Note: threaded=True for concurrent request handling
